...
Bump rationale: MINOR——原則 III 新增「補位」條款：三桶精選完成配額後未滿 15 則時，以「未歸類高熱度」候選中的
  資安事件與一般軟體工程新聞補位（永遠排最後、只補到上限、不計入非 AI 配額）。屬既有原則內的機制擴充，未移除或
  重新定義原則，故非 MAJOR；領域聚焦條文不變。
  決策來源（2026-09-26）：1.7.0 上線首日晨報只有 7 則，未歸類 10 則中有 F-Droid 2.0、Snapdragon 支援 Linux、
  FBI 被駭等對開發者有價值的新聞，因不在三桶範圍而全數不選。使用者定位：這兩類本質上就是「未歸類高熱度」，
  只在則數偏低、且當日沒有更優先的 AI 新事物時才候補，不是主動擴大範圍。
Modified sections: 原則 III（新增補位條款）、原則 VIII（必測項新增補位規則）。
Templates requiring updates: CLAUDE.md ✅、dev-guide §4.4 ✅、README ✅、src/curation（prompt／parse／validate）✅、
  src/state/state.schema.ts（domain 列舉加 general）✅、相關測試 ✅。

Prior history:
  - 1.7.0（2026-09-25）：MINOR——晨報上限 10 → 15、非 AI 預設 3 → 5、動態讓額改 `max(5, 15 − AI)`；原則 VIII
    新增未歸類高熱度通道與同題群集必測項。起因 Jev（HN 1979 分）連續四天被 cross 關鍵字歸類丟棄。
...
- **新聞晨報每日固定精選至多 15 則**：**AI 不設量化下限**，逐一評選「是否真的重要」、合格皆收錄；
  **DevOps / 後端 / 前端合計預設上限 ≤ 5**，若當日 AI 則數不足 10、使總數未達 15，非 AI 上限可放寬
  ...
  候選不足自然少推）。
- **補位（1.8.0）**：三桶精選（含未歸類候選中的 AI 新事物，優先度最高）完成配額後若未滿 15 則，才以
  「未歸類高熱度」候選中的**資安事件**與**一般軟體工程**新聞依序補位——補位永遠排在最後、只補到上限、
  不計入非 AI 配額，與軟體開發無關者仍一律不收。這是則數偏低時的補位機制，**不是**主動擴大晨報的
  領域範圍；領域聚焦（下一條以下）不變。
...
**同題群集判定**、**補位（只收未歸類候選、只補到上限、排在最後、不計入非 AI 配額）**、**70 / 500 / 250 字數上限驗證**、
...
**Version**: 1.8.0 | **Ratified**: 2026-07-11 | **Last Amended**: 2026-09-26
