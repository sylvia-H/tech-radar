...
Bump rationale: MINOR——原則 III 的**新聞晨報 AI 下限「≥7」改為不設量化下限（逐一評選、合格皆收），
  DevOps/後端/前端上限由固定 `≤3` 改為動態：AI 當日不足 7 則、使總數未達 10 則時，非 AI 上限放寬至
  `10 − AI 則數`，把 AI 未用滿的名額讓給非 AI**。屬既有原則內的參數/機制調整（先例：1.4.0、1.5.0
  同樣調整原則 III 的新聞配額參數，判 MINOR），未移除或重新定義八條原則本身，故非 MAJOR；因調整
  的是 NON-NEGOTIABLE 原則本身的配額機制（非僅技術約束細節），故非 PATCH。

  決策來源（2026-08-04）：比對 2026-07-19～08-03 每日實際推播則數，發現 `MIN_AI=5` 期間每天約
  5～6 則、改為 `MIN_AI=7` 後連續 5 天**逐日精確命中 7、不多不少**——判定這是 LLM 對 prompt 中
  具體數字的錨定（anchoring）：模型把「軟性下限」讀成「目標產出量」，10 則上限形同虛設，
  「寧缺勿濫」的設計意圖未真正運作。移除 prompt 中所有 AI 則數的具體數字，改為純定性指示
  （逐一評估是否真的重要、合格皆收錄、不要因為湊到「還可以」的數量就提早停手）。

  移除數字錨點後，AI 側改為完全依候選品質產出、不再保證達到原本的 7 則下限；為避免 AI 表現不佳
  的日子連帶讓整份晨報則數偏少（例如 AI 僅 4 則、非 AI 硬性 ≤3，總數僅 7 則而非可能的 10 則），
  新增非 AI 上限的動態讓額機制：AI 未用滿「10 − 非 AI 上限(3)」＝7 則的隱含額度時，非 AI 可承接
  AI 讓出的名額，上限公式 `max(3, 10 − AI 則數)`——AI ≥7 時等同原本固定 ≤3，AI <7 時等比放寬。

  動機：讓「10 則」這個總量上限盡量被填滿，同時不犧牲「AI 為主」的領域配置精神——非 AI 只在
  AI 確實供給不足時才獲得額外名額，不是無條件放寬。

Modified sections:
  - 原則 III「只推變化、控制節奏」→ AI 下限「≥7」→ 不設量化下限；非 AI 上限「固定 ≤3」→
    「預設 ≤3，AI 不足 7 則時放寬至 10−AI 則數」。
  - 原則 VIII「關鍵邏輯測試優先」→ 必測項「新聞配額」描述同步更新為新機制，並新增「非 AI 動態
    讓額」為必測項。
Added sections: 無
Removed sections: 無

Templates requiring updates:
  - CLAUDE.md ✅ 已同步（硬規則 3 之新聞配額描述與調整歷程）
  - docs/tech-radar-dev-guide.md ✅ 已同步（§4.4 過濾漏斗與晨報精選、§5.3 推播組成、
    §7.2 組版範例程式碼註解、§11.1 Constitution 摘要段、§12 風險與坑新增 anchoring 教訓）
  - src/curation/curation-quota.ts ✅ 已同步（`MIN_AI` 常數移除；新增 `clampNonAi` 動態上限公式）
  - src/curation/curation-prompt.ts ✅ 已同步（prompt 內 AI 則數數字全數移除，改純定性指示）
  - src/curation/curation-validate.ts ✅ 已同步（`clampNonAi` 呼叫改依 AI 則數動態決定上限）
  - 相關測試（curation-quota.spec.ts、curation-validate.spec.ts）✅ 已同步
  - specs/006-news-curation 之既有 spec/plan/quickstart ⚠ 維持原樣不改——歷史 Feature 已驗收
    合併，屬完成當時的歷史紀錄（沿用本專案「歷史 spec 不回改，只更新真實來源」慣例，見 CLAUDE.md）
...
- **新聞晨報每日固定精選至多 10 則**：**AI 不設量化下限**，逐一評選「是否真的重要」、合格皆收錄；
  **DevOps / 後端 / 前端合計預設上限 ≤ 3**，若當日 AI 則數不足 7、使總數未達 10，非 AI 上限可放寬
  至 `10 − AI 則數`，把 AI 未用滿的名額讓給非 AI（軟性上限，寧缺勿濫、不硬湊）。
...
**新聞配額（至多 10 則，AI 不設下限逐一評選；DevOps+後端+前端預設 ≤3，AI 不足 7 則時動態放寬至
10−AI 則數）**、**70 / 500 / 250 字數上限驗證**、
...
**Version**: 1.6.0 | **Ratified**: 2026-07-11 | **Last Amended**: 2026-08-04
