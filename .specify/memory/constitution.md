...
Bump rationale: MINOR——原則 III 的**新聞晨報總數上限由「至多 10 則」放寬為「至多 15 則」，DevOps/後端/前端
  預設上限由 ≤3 放寬為 ≤5，動態讓額公式由 `max(3, 10 − AI 則數)` 改為 `max(5, 15 − AI 則數)`（AI 隱含額度
  7 → 10）**；原則 VIII 必測項新增「未歸類高熱度通道」與「同題群集」。屬既有原則內的參數調整與指引擴充
  （先例：1.4.0／1.5.0／1.6.0 同樣調整原則 III 的新聞配額，判 MINOR），未移除或重新定義原則，故非 MAJOR。

  決策來源（2026-09-25）：使用者反映 09-15 起 Jev（TypeSafe AI 的 System One 決策模型）崛起、晨報全無。
  診斷：HN「Introducing System One Models and Jev」1979 分在四天視窗內每日被抓到、每日被 `cross` 關鍵字歸類
  （標題無任何 AI 關鍵字 → null）靜默丟棄；重播 09-16／19／22／25 的 HN top 100，關鍵字歸類每日丟 72～85%，
  ≥300 分被丟者 38～53 則，含 MiMo v2.6、OpenJev、Grok 4.7、Qwen 3.8 Omni Flash 等新模型發布。問題是系統性
  的。使用者原則：**高熱度但關鍵字歸類不到的候選不是雜訊，而是晨報最核心的價值——「我不知道、但應該關注」
  的資訊**。據此新增「未歸類高熱度通道」（關鍵字只負責正向命中、不再有否決權；`score ≥ 300` 者以「未歸類」
  入池、每日至多 10 則、交策展 LLM 判定領域與重要性並回填 `domain`）與零 LLM 的「同題群集」訊號（同一罕見詞
  跨 ≥2 來源出現 ≥3 則即標記給 LLM）。預期合格候選增加，故總數上限 10 → 15、非 AI 5、AI 隱含 10——上限是
  天花板不是目標（prompt 仍不設任何下限、逐一評選），候選不足自然少推、沒有硬湊的損失。同時策展主備型號改為
  Flash-Lite 系（`gemini-3.5-flash-lite` 主、`gemini-3.1-flash-lite` 備援，dev-guide §2.4），以承接候選池
  60 → 70 與輸出 15 則的用量。

Modified sections:
  - 原則 III「只推變化、控制節奏」→ 新聞晨報「至多 10 則／非 AI 預設 ≤3／AI 不足 7 則時放寬至 10−AI」→
    「至多 15 則／非 AI 預設 ≤5／AI 不足 10 則時放寬至 15−AI」。
  - 原則 VIII「關鍵邏輯測試優先」→ 必測項「新聞配額」數字同步；新增「未歸類高熱度通道（分數門檻、每日名額、
    降級路徑排除）」與「同題群集判定」。
Added sections: 無
Removed sections: 無

Templates requiring updates:
  - CLAUDE.md ✅ 已同步（硬規則 3 配額描述與調整歷程；補「關鍵字歸類只負責正向命中」守則）
  - docs/tech-radar-dev-guide.md ✅ 已同步（§4.3 歸類、§4.4 漏斗第 8 步／階段 B、§5.3、§7.2、§11.1、§12 教訓）
  - README.md ✅ 已同步（一眼看懂表、第 4／6／7／8／9 關、工程亮點）
  - src/curation/curation-quota.ts ✅（`MAX_ITEMS`=15、`MAX_NON_AI`=5）；src/news/funnel.ts ✅（未歸類通道常數）
  - 相關測試 ✅ 已同步（curation-quota／validate／fallback／prompt、funnel、news-ingest、topic-cluster）
  - specs/00x 之既有 spec/plan/quickstart ⚠ 維持原樣不改（歷史 Feature 已驗收合併，沿用「歷史 spec 不回改、
    只更新真實來源」慣例）

Follow-up TODOs: 觀察一週（至 2026-10-02）：未歸類候選入池數與入選數、同題群集是否命中新事件、Lite 主型號的
  則數是否偏低或輸出截斷（`LlmError('empty')`）、候選池是否觸頂 70。

Prior history:
  - 1.6.0（2026-08-04）：MINOR——原則 III 的新聞晨報 AI 下限「≥7」改為不設量化下限（逐一評選、合格皆收），
    非 AI 上限由固定 ≤3 改為動態 `max(3, 10 − AI 則數)`。決策來源：`MIN_AI=7` 期間 LLM 連續多日精確命中 7 則
    （prompt 數字錨定，把軟性下限讀成目標產出量），遂移除 prompt 中所有 AI 則數數字、改純定性指示，並以動態
    讓額避免 AI 表現不佳的日子整份偏少。
  - 1.5.0（2026-07-29）：MINOR——原則 III 的新聞晨報 AI 下限由「≥5」提高為「≥7」，且字數上限由
    「標題 ≤50 / 內容 ≤300」放寬為「標題 ≤70 / 內容 ≤500」（repo 簡介 ≤250 不變）。決策來源：
    使用者發現 1.4.0 上線後晨報實際每天仍只收到約 5 則新聞，要求把 AI 下限提高到 7 以加重措辭
    力道；同時認為 300 字內容偏短，要求放寬到 500 字。
...
- **新聞晨報每日固定精選至多 15 則**：**AI 不設量化下限**，逐一評選「是否真的重要」、合格皆收錄；
  **DevOps / 後端 / 前端合計預設上限 ≤ 5**，若當日 AI 則數不足 10、使總數未達 15，非 AI 上限可放寬
  至 `15 − AI 則數`，把 AI 未用滿的名額讓給非 AI（軟性上限，寧缺勿濫、不硬湊；上限是天花板不是目標，
  候選不足自然少推）。
...
**新聞配額（至多 15 則，AI 不設下限逐一評選；DevOps+後端+前端預設 ≤5，AI 不足 10 則時動態放寬至
15−AI 則數）**、**未歸類高熱度通道（`cross` 關鍵字無命中者的分數門檻、每日名額、降級路徑排除）**、
**同題群集判定**、**70 / 500 / 250 字數上限驗證**、
...
**Version**: 1.7.0 | **Ratified**: 2026-07-11 | **Last Amended**: 2026-09-25
