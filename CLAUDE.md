...
3. **只推變化、控制節奏（III）**：榜單**每七天推一次且只呈現差異**（`lastBoardPushAt` 計時，非
   cron；到期門檻 **162h** = 168h − 6h 寬限）；新聞**每日固定精選至多 10 則**（AI **不設量化下限**、
   逐一評選、合格皆收；DevOps/後端/前端預設合計 **≤3**，若當日 AI 則數不足 7、使總數未達 10，
   非 AI 上限可放寬至 `10 − AI 則數`，把 AI 未用滿的名額讓給非 AI；寧缺勿濫、不硬湊；2026-07-19
