...
3. **只推變化、控制節奏（III）**：榜單**每七天推一次且只呈現差異**（`lastBoardPushAt` 計時，非
   cron；到期門檻 **162h** = 168h − 6h 寬限）；新聞**每日固定精選至多 15 則**（AI **不設量化下限**、
   逐一評選、合格皆收；DevOps/後端/前端預設合計 **≤5**，若當日 AI 則數不足 10、使總數未達 15，
   非 AI 上限可放寬至 `15 − AI 則數`，把 AI 未用滿的名額讓給非 AI；寧缺勿濫、不硬湊；2026-07-19
   由 6 則/AI≥4/非AI≤2 調整為 10 則/AI≥5/非AI≤3，2026-07-29 AI 下限再調整為 ≥7，2026-08-04 因
   `MIN_AI` 於 prompt 中觸發 LLM 數字錨定（實測連續多日精確卡在 7 則）而拿掉 AI 固定下限、改為
   上述動態讓額機制，2026-09-25 因新增「未歸類高熱度通道」預期合格候選增加而由 10／3 調整為
   15／5（憲章 1.7.0），見 dev-guide §4.4）；字數上限
   **標題 ≤ 70 / 內容 ≤ 500 / 簡介 ≤ 250**（2026-07-29 由 50/300 放寬，簡介 250 不變）；
   一律繁體中文；按對開發者重要性排序（非熱度）。
   **`cross` 來源的關鍵字歸類只負責正向命中、不得否決高分項目**（2026-09-25，使用者原則：高熱度但
   關鍵字不認識的候選正是「我不知道、但應該關注」的新聞本身）：無命中且分數 ≥300 者以「未歸類」入池、
   交策展 LLM 判定領域，勿再把「無關鍵字＝離題」寫回程式（dev-guide §4.3／§4.4 第 8 步）。
   **補位（憲章 1.8.0，2026-09-26）**：三桶精選（含未歸類候選中的 AI 新事物）完成配額後若未滿 15 則，才以「未歸類高熱度」候選中的資安事件與一般軟體工程新聞依序補位，補位永遠排在最後、只補到上限、不計入非 AI 配額；這是補位機制，不是主動擴大晨報範圍（`backfillPicks`，領域記 `general`）。
