...
- [ ] （同一週，決策用）**未歸類入選內容的主題分布**：逐日記下未歸類候選被 LLM 選入的每一則屬於哪一類——
      (a) AI 新模型／新工具（三桶範圍內、只是名字陌生）、(b) 資安事件／漏洞、(c) 一般軟體工程（語言／執行環境版本、
      資料庫、工具鏈）、(d) 監管政策／產業事件、(e) 與開發無關但 LLM 仍選了（誤選）。判讀：(a) 為主 → 三桶範圍正確、
      通道即可；(b)(c) 佔多數 → 這是 F9「科技／科學新聞第二晨報」（或第四桶）的證據，屆時先修憲章 III（領域聚焦）
      與 V（每日一次呼叫）再開 Feature；(e) 反覆出現 → 收緊 prompt 的「與開發無關者不選」段或提高 `unresolvedMinScore`。
      同時看 max-items 剔除是否天天出現（15 則上限是否被當目標）。
- 候補改動（觀察後再決定）：把「LLM 可回填 `domain`」由未歸類候選擴大到**所有 cross 來源候選**（HN、lobsters-programming），
  對這兩個來源以 LLM 判定的領域為準、其餘來源仍用設定檔——關鍵字對 HN 是弱證據卻有最終決定權（「Jev in 25 Lines of
  Python」只因命中 python 被歸前後端）。屬判斷而非事實數據，不牴觸憲章 VI。
