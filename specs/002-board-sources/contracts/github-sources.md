...
- **輸出**：`TrendingResult { repos: RawTrendingRepo[]; failedPages: string[] }`（見 data-model）。跨 6 頁先以 `fullName` 去重。
- **topics 補抓**：Trending HTML 不含 topics → 每個唯一候選再打「§3 repos」。
- **逐頁隔離**（FR-007）：6 頁**各自 try/catch**。單頁抓取失敗（重試耗盡）或該頁任一列欄位抽不到 → 該頁記入 `failedPages`（全站頁記 `all`）、送告警 `source=github-trending:{page}`，**其餘頁的候選照常合併入榜**。爆炸半徑限縮在該頁：一頁 404 不得讓另外 5 頁的候選陪葬。
- **失敗與 0 筆**（FR-009）：**全部頁合併後 0 筆**（含 6 頁全失敗）→ 擲可辨識錯誤 → 送告警 `source=github-trending`，**不**視為「本週無熱門」，主力候選當空、補位仍續行。
- **stars 取值**：`.float-sm-right` 文字 `replace(/[^0-9]/g,'')` 後**必須非空**，否則擲錯（頁面改版）。不得改用 `Number.isFinite` 當守衛——`Number('')` 是 `0` 不是 `NaN`，該守衛恆為真，改版把數字挪走時會靜默記成 0 星。
...
- **查詢**（三組，各一次；門檻 clarify 已定）。OR 群 **MUST 衍生自 `DOMAIN_KEYWORD_SETS[domain].search`**，不得在本服務另抄字面量（見 data-model「領域關鍵字種子集」）：
...
- **退避**：5xx / 429 / **限流型 403** / 網路錯誤 → 指數退避＋jitter（憲章 VII、§12）。GitHub 的 secondary rate limit 與限額耗盡可能回 **403**，以帶 `Retry-After` 或 `X-RateLimit-Remaining: 0` 辨識；**憑證型 403**（無此特徵）與 401/404/422 維持不重試、直接擲帶狀態碼的錯誤（見 research D9）。
- **rate-limit 監看**：`X-RateLimit-Remaining` **header 缺席時略過判斷**（`Number(null)` 為 `0`，逕自轉數字會誤判成逼近限額）。
- **呼叫計數**（SC-006）：於**送出前**累加，失敗與重試皆算同一次邏輯呼叫（失敗一樣吃限額，不得低估）。
