# Feature Specification: 榜單來源與兩領域歸類（Board Sources）
...
本功能的「使用者」即專案擁有者（自用）。目標是把「近一週最受關注、新崛起的 GitHub repo」轉成一份分好兩領域、可觀測的**當前榜單**，作為後續變化偵測（F3）、簡介（F5）與推播（F7）的資料地基。此階段**只產出當前榜單快照供觀測，不做跨執行的變化比對，也不推播到 Discord**。

### User Story 1 - 兩領域週增星榜可被觀測（Priority: P1）

擁有者執行任務後，能在執行紀錄（log）看到 **AI / 前後端** 兩領域各自的當前榜單，每項含 repo 名稱、本週增星量、所屬領域與排序位置，且榜單內容與實際本週熱門情形相符。

**Why this priority**: 這是里程碑 M1 的核心價值，也是「只看變化」（F3）與其後所有功能的前置。沒有一份可信、分好領域的當前榜單，之後的 diff、簡介與推播都無所依附。它本身即構成可展示的最小可用產品：擁有者能看到「這週兩領域各有哪些 repo 在竄升」。

**Independent Test**: 於已備妥 GitHub token 的環境執行一次任務，檢視 log 是否印出兩領域榜單、每項欄位齊備（名稱／本週增星／領域／名次），並人工抽查數個 repo 的領域歸類與週增星數是否合理；不依賴狀態快照、LLM 或 Discord 即可完整驗證。

**Acceptance Scenarios**:

1. **Given** GitHub token 已設定且來源可達，**When** 擁有者執行任務，**Then** log 印出 AI / 前後端兩領域的當前榜單，每領域至多 15 筆，每筆含 repo 識別（owner/name）、本週增星量、所屬領域與名次。
...
**Independent Test**: 只啟用補位來源執行一次，檢視是否取得「建立時間在近 7 天內、且已累積一定星數」的 repo，並確認這些 repo 能被歸入兩領域；與主力來源分開即可驗證。

**Acceptance Scenarios**:

1. **Given** 近 7 天內有新建且已累積相當星數的 repo，**When** 補位來源查詢執行，**Then** 這些 repo 被納入候選並依領域關鍵字分組查詢（AI／前後端各一組）。
...
- **跨領域 repo**：同時符合多個領域關鍵字的 repo，**擇一主領域，優先序 AI > 前後端**，只入單一領域榜（見 FR-011）。
...
- Q: 榜單範圍是三領域還是全網熱門？ → A: 維持 **AI / DevOps / 前後端 三領域**（沿用憲章對本專案的定義，非全網熱門），但歸類規則採**寬鬆傾向**，以免漏掉確屬三領域的好工具（如 codegraph 這類開發者工具本就在範圍內）。
  > **⚠️ DevOps 已於 Session 2026-07-15 移除**：榜單收斂為 **AI / 前後端 兩領域**。以現行 FR-003 為準。
- Q: 一個 repo 的 topics／關鍵字同時命中多個領域時如何歸屬？ → A: **擇一主領域**，依固定優先序 **AI > DevOps > 前後端**，只入單一領域榜（去重乾淨、名額不被同一 repo 重複佔用）。
  > **⚠️ Session 2026-07-15 更新**：DevOps 移除後優先序為 **AI > 前後端**（見 FR-011）。
...
- Q: 補位 Search 星數門檻與三領域關鍵字集是否採開發指南 §3.2 種子集？ → A: **採用 §3.2 為 v1 canonical**（AI `stars:>30`、DevOps／前後端 `stars:>20`、`created:>今天−7天`；關鍵字種子集見「領域關鍵字種子集」），可日後擴充、只改設定不改 pipeline。
  > **⚠️ Session 2026-07-15 更新**：DevOps 組查詢隨領域移除而取消，Search 由 3 組減為 **2 組**（AI `stars:>30`、前後端 `stars:>20`）。
...
- Q: 綜合 top 10 要不要保證三領域都有代表？ → A: **保底每領域至少 2 席**，其餘名額比熱度；推播卡片 **MUST 標示週增星數**以呈現人氣落差。
  > **⚠️ Session 2026-07-15 更新**：榜單改兩領域後，追蹤深度由 45 筆（3×15）變 **30 筆（2×15）**，保底席次由 6 席（3×2）變 **4 席（2×2）**，其餘 6 席比熱度。規則本身不變，屬 F3/F7 實作。
...
- **I1 跨領域決勝機制**：language **不參與**跨領域主領域決勝；跨領域一律依 **FR-011 固定優先序 AI > DevOps > 前後端**（**Session 2026-07-15 起為 AI > 前後端**）。language 僅為輔助訊號、不單獨定領域、不改變領域歸屬（FR-003 已據此收斂；解消與研究 D4「語言加分跨領域決勝」之張力）。
...
#### 榜單移除 DevOps 領域（範圍變更，擁有者 2026-07-15 定案）

M1 驗收的人工抽查（SC-002）以真實資料發現 **DevOps 榜的歸類正確率為 0**，據此收斂榜單為 **AI / 前後端 兩領域**：

- **證據**：當次 DevOps 榜僅 3 筆，全部只靠 `docker` 命中，且全非 DevOps 工具——`docker-steam-headless`（topics `docker, steam, gameserver…`，在容器裡跑 Steam 遊戲）、`docker-mailserver`（郵件伺服器）、`teledrive`（topics `react, python, docker, typescript, vite, fastapi`，實為 React+FastAPI 檔案管理器，因 `docker` 命中 DevOps 又被 FR-011 優先序自前後端榜搶走，屬 SC-002 明文禁止的「明顯錯置」）。**根因**：`docker` 是「部署方式」標籤而非「領域」標籤，self-hosted 應用幾乎都貼。
- **另一理由**：DevOps 候選的週增星只有 259／136／25，對比 AI 榜的 13,195／7,129，即使 F3/F7 有保底席次也排不上號——保留一個高誤收、低訊號的領域，只是稀釋榜單。
- **僅收窄 `docker` 不足**：實測移除 `docker` 後 `docker-steam-headless` 排除、`teledrive` 正確歸前後端，但 `docker-mailserver` 因真的貼了 `kubernetes` 仍留在榜上——「能跑在 k8s 上」與「是 k8s 工具」用純關鍵字（零 LLM，憲章 V）無法區分，屬固有極限。故直接移除領域，而非繼續調關鍵字。
- **範圍**：**僅榜單**。新聞側的 DevOps 領域、配額（憲章 III「AI ≥ 4；DevOps/後端/前端合計 ≤ 2」）與三個 DevOps 專屬來源（Reddit r/devops、The New Stack、Lobste.rs `/t/devops.rss`）**皆不受影響**——新聞是另一條資料流，「DevOps 沒有爆紅 repo」不等於「沒有值得讀的 DevOps 消息」，且新聞本就以「重要度優先於熱度」排序。
- **連帶影響**：`Domain` 收為 2-way；種子集移除 devops 群（`docker` 問題隨之消失）；Search 由 3 組減為 **2 組**（SC-006 更寬鬆）；FR-011 優先序為 **AI > 前後端**；追蹤深度 45→**30 筆**、F3/F7 保底席次 6→**4 席**；F3 對齊 `state.schema.ts` 時須一併移除 `devops`。憲章、CLAUDE.md、dev-guide 之**榜單**領域敘述同步修訂（新聞敘述不動）。
...
- **FR-002**: 系統 MUST 從 GitHub Search 取得「建立時間在近 7 天內、且已累積一定星數」的補位候選，並針對 AI／前後端分別查詢，以補足尚未擠上主榜的新崛起 repo。
- **FR-003**: 系統 MUST 將候選 repo 歸入兩領域之一：**AI / 前後端**。歸類**主要訊號為 repo 的 topics**；當 repo **無 topics** 時，MUST 退而以 **description** 比對兩領域關鍵字集合。主要語言（language）**僅為輔助訊號、MUST NOT 單獨決定領域，且 MUST NOT 改變跨領域主領域之判定**（未因 topics/description 命中任何關鍵字者，即使語言相符也 MUST 被排除；命中多領域時，主領域一律依 FR-011 固定優先序決定，language 不參與該決勝，至多作為「已命中單一領域」時的信心佐證，不改變領域歸屬）；topics 與 description 皆無命中者 MUST 被排除，不強行歸類（寧缺勿濫）。**比對語意**（正規化為小寫後）：topics 與 description **一律以詞界比對**（以非英數字元為界，keyword 內部連字號不受影響）——短關鍵字若對 topics 用子字串，`ai` 會命中 `blockchain`／`domain-driven-design`／`training`，再被 FR-011 的 AI 最高優先序吃掉歸類，直接侵蝕 SC-002（見 Clarifications「Session 2026-07-15」A1 修訂）。**寬鬆傾向**改由種子集承擔：詞界比對接不到的黏著變體（`openai`／`genai`／`agents`／`reactjs` 等）MUST 逐一列入關鍵字種子集的補充群，確屬兩領域的開發者工具（工具鏈／框架／平台等，如 codegraph）不應因關鍵字過嚴而被漏收；增刪仍只改設定、不動 pipeline。
...
- **FR-006**: 系統 MUST 將產出的兩領域當前榜單以可觀測方式輸出（log 可印出，含 repo 識別、本週增星量、領域、名次），供人工驗收與後續 Feature 取用；本 Feature **MUST NOT** 進行狀態快照寫回、變化偵測或 Discord 推播。
...
- **FR-010**: 本 Feature 的關鍵設定已於 `/speckit-clarify`（Session 2026-07-11）定案並回填如下，屬**可日後擴充的 v1 canonical**（調整只改設定、不改 pipeline）：
  ...
  - **補位 Search 星數門檻**（沿用開發指南 §3.2）：AI `stars:>30`、前後端 `stars:>20`，皆配合 `created:>今天−7天`（DevOps 組已隨領域移除，Search 為 2 組）。
  - **跨領域歸屬**：見 FR-011（同時命中多領域時擇一主領域之固定優先序）。
  - **兩領域關鍵字種子集（v1）**：見下方「領域關鍵字種子集」。
- **FR-011**: 當一個 repo 同時命中多個領域時，系統 MUST 依固定優先序 **AI > 前後端** 擇一**主領域**，該 repo 只出現在**單一領域榜**、不同時入多榜（確保去重乾淨、每領域 top 15 名額不被同一 repo 重複佔用）。
...
> **DevOps 群已於 Session 2026-07-15 隨領域移除**（原為 `kubernetes`、`terraform`、`gitops` ＋ `devops`、`ci-cd`、`docker`、`observability`、`platform-engineering`）。**此處僅指榜單**；新聞側的 DevOps 領域、配額與來源不受影響（憲章 III，另一條資料流）。
...
- **領域（Domain）**：榜單的兩個分類之一——**AI / 前後端**（程式列舉值 `ai` / `frontend-backend`）。由關鍵字／主題集合定義（見「領域關鍵字種子集」）。
- **當前榜單（Current Board）**：每領域一份、排序後至多 15 筆候選 repo 的清單，代表「本次執行觀測到的近一週兩領域熱門與新崛起」。為本 Feature 的最終產出（僅供觀測，不持久化為狀態快照——狀態快照屬 F3）。

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: 執行一次任務後，擁有者能在執行紀錄看到 **AI / 前後端兩領域**的當前榜單，每領域至多 15 筆且欄位齊備（repo 識別、本週增星量、領域、名次）。
- **SC-002**: 對榜單中的 repo 人工抽查，**至少 90%** 的領域歸類判斷為合理（歸對領域），無明顯錯置。
- **SC-003**: 同一個 repo（含改名情境）在最終兩領域榜單中**只出現一次**，去重正確率 100%（以含重複與改名的樣本驗證）。
...
- **兩領域定義沿用專案既定分類**：AI / 前後端（前端 + 後端合為一領域，與卡片領域配色一致）。**關鍵字／主題種子集已於 clarify 定案**（見 FR-010 與「領域關鍵字種子集」），為可日後擴充的 v1。
...
- **榜單領域為 2-way、與 F1 持久化 schema 對齊留待 F3**：本 Feature 分類輸出的領域型別為二值 `ai` / `frontend-backend`（顯示「前後端」）。F1 `state.schema.ts` 之 `BoardEntry.domain` 目前為 4-way 佔位（`ai|devops|backend|frontend`），因 **F2 不寫回狀態**故不在本 Feature 修改；持久化層對齊 2-way 留待 **F3**（首次寫回 board 時，符合 F1「domain enum 值在 F2 clarify 定案」之預期）。**注意 F3 對齊時 `devops` 亦須一併移除**（榜單已無此領域）。
...
- **補位來源查詢門檻**：**已於 clarify 定案採開發指南 §3.2 門檻**（AI 星數 > 30、前後端星數 > 20，建立時間 > 今天−7 天）（見 FR-010）。
