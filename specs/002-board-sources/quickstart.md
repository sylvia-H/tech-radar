# Quickstart: 驗證 F2 榜單來源與兩領域歸類（M1）

驗收目標（M1）：**本機／Actions log 印出正確的兩領域週增星榜**。本指引為驗證/執行步驟，實作細節見 [plan.md](plan.md)、[data-model.md](data-model.md)、[contracts/](contracts/)。
...
- **分類**：topics 命中歸對領域；無 topics 靠 description；只有語言相符不歸類；跨領域擇一主領域（AI>前後端）；皆無命中→排除（FR-003/FR-011）。
...
- 印出 **AI / 前後端** 三段，每段 ≤15 筆，每筆含**名次、`owner/name`、`weeklyStarsEstimate`、來源、領域**。
...
- 隨機挑 **≥10 個** repo（跨兩領域），開其 GitHub 頁確認：領域歸類合理（**目標歸對率 ≥90%**，記錄實際比率）、`weeklyStarsEstimate` 與實際本週熱度相符（Trending 筆為官方週增星）。
...
- `workflow_dispatch` 觸發 radar workflow → 於 Actions log 看到同樣兩領域榜輸出；確認未產生非預期的 `state/board.json` commit（F2 不寫狀態）。

---

**通過標準**：步驟 1 全綠 ＋ 步驟 2 log 印出欄位齊備的兩領域榜 ＋ 步驟 3 抽查歸類合理 ＋ 步驟 4 容錯告警帶來源 id。達成即滿足 M1，可進入 F3（`003-board-state-diff`）。
