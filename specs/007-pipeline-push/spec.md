...
Discord 單則訊息最多 10 個 embeds。F7 依顯示順序（榜單封面 → 卡片… → 晨報）把 embeds 切成每則 ≤10 的
批次送出。穩定態每七天多半 0～數張卡，一則訊息即可容納；**冷啟動日**全數新進（10 張卡）＋封面＝11 個
embeds，**超過單則上限**，必須正確跨訊息拆分（不得一次送 >10 使 Discord 拒收）。晨報 6 則若逼近
description 4096 上限，拆成兩張晨報 embed（同批次內）。

**Why this priority**: 這是 dev-guide §7.1/§7.2 的版面規格落地。優先序 P3：穩定態極少觸發拆分，屬邊界情形；
但**冷啟動當次會實際超過 10 embeds**，若不處理會被 Discord 整則拒收而推播失敗，故仍須正確。與 US1～US3
可獨立驗證：構造「封面＋10 張卡＋晨報」的 embed 集合，斷言切分後每則 ≤10、順序不亂、無 embed 遺漏。

**Independent Test**: 構造冷啟動 embed 集合（封面 1＋新進卡 10＋晨報 1＝12 個 embeds），呼叫組版／送出，
斷言：切成多則、每則 ≤10、顯示順序保持、總 embed 數不增不減；再構造穩定態（封面＋2 卡＋晨報＝4）斷言
單則送出。全程 mock 推播、只檢查批次。

**Acceptance Scenarios**:

1. **Given** 任一次要送出的 embeds 總數 ≤10，**When** 送出，**Then** 以**一則**訊息送出。
2. **Given** embeds 總數 >10（如冷啟動封面＋10 卡＋晨報），**When** 送出，**Then** 依顯示順序切成多則、每則
   **≤10** embeds、無遺漏、無重複，任一則**MUST NOT** 送出 >10 個 embeds。
3. **Given** 晨報 6 則的 description 逼近 4096 字元上限，**When** 組晨報 embed，**Then** 拆成兩張晨報 embed
   （仍納入 ≤10 切分規則），避免單一 description 超限被拒。
...
- **FR-017**: 送往 Discord 的訊息 MUST 每則 ≤10 embeds：系統 MUST 依顯示順序（榜單封面 → 卡片… → 晨報）
  將 embeds 切成每則 ≤10 的批次送出；冷啟動（封面＋10 卡＋晨報＝12）等 >10 情境 MUST 正確跨訊息拆分、
  無遺漏無重複、順序不亂，任一則 **MUST NOT** 送出 >10 embeds。此通用切分涵蓋 dev-guide §7.2「晨報改送
  第二則」的特例。
...
- **SC-005**: 送往 Discord 的**任一則訊息 embeds 數 ≤10**——含冷啟動（封面＋10 卡＋晨報＝12）情境，>10 的
  送出次數為 **0**；切分後 embed 總數不增不減、顯示順序保持。
...
- **push-then-commit 取代 F3 現行 log-success→commit**：F3 已預留此接縫（`commitBoardPush` 純函式與其測試
  ...
  （快取效益只在推播成功後兌現，憲章 VI）。
- **embed ≤10 通用切分涵蓋 dev-guide §7.2 特例**：dev-guide §7.2「封面＋10 卡留在第一則、晨報送第二則」的
  敘述在**冷啟動恰為 11 embeds**時仍超過單則 10 上限；F7 採**依序 chunk-by-10** 的通用規則（更簡單且正確），
  §7.2 該敘述視為特例、實作以通用切分對齊（此屬 F7 內部組版細節、不影響其他 Feature）。
