...
- [X] `npm test` 全綠，涵蓋 [data-model.md §5](data-model.md) 全部規則。
- [X] 冷啟動印出 10 筆新進、0 竄升、0 下降，狀態寫入 ≤10 筆（§2）。
- [X] 再次到期時輸出「榜單無變化」或只含差異項（§3）。
- [X] 未到期時整段跳過、無任何抓取、狀態不變（§4）。
- [X] 空榜時告警 + 狀態逐位元組不變 + 不中斷（§5）。
