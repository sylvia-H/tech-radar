...
- **FR-014**: 排程層 MUST 以 workflow 層級（`if: failure()`）在**應用程式自身邏輯以外**的任何失敗時補送一則紅色告警——涵蓋 app 啟動前（checkout、建置、機密載入導致 app 未能執行）**以及 app 成功結束後的狀態 commit/push 階段**失敗，確保任何層級的失敗都不會無聲。
