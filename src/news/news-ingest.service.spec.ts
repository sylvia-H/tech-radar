...
describe('NewsIngestService.ingest — 排除已見於收斂之前（Fix 1）', () => {
  // 分散在 21 個來源（每源 3 篇），避免觸發同來源上限 maxNullScorePerSource=3（2026-08-04 新增）。
  const sources: NewsSource[] = Array.from({ length: 21 }, (_, i) => ({
    id: `good-rss-${i}`,
    type: 'rss' as const,
  ...
  // 61 筆（> convergeMax 60，2026-09-14 由 50 調高），依 normalizedUrl 遞增天然排序；i=0 最新、i=60 最舊
  // （發文時間本身不影響排序，僅供標題/連結區隔）。
  ...
  const parse = (xml: string) => {
    ...
    return { items: sixtyOne.slice(idx * 3, idx * 3 + 3) };
  };

  it('最新一筆已見時：先排除再收斂 → 仍輸出 60 筆，且排名其後的新鮮候選不被排擠', async () => {
    const state: BoardState = {
      ...emptyBoardState(),
    ...
    // 先排除已見 p00（剩 60）→ 收斂上限 60 → 全數保留；最舊的 p60 不因先收斂而被排擠掉。
    expect(out).toHaveLength(60);
    const urls = out.map((c) => c.normalizedUrl);
    expect(urls).not.toContain(normalizeTargetUrl('https://good.example/p00'));
    expect(urls).toContain(normalizeTargetUrl('https://good.example/p60'));
  });
});
