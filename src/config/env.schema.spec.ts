...
describe('env.schema validateEnv', () => {
  it('合法環境變數通過並回傳解析值', () => {
    ...
    expect(parsed.DISCORD_WEBHOOK_URL).toBe(validEnv.DISCORD_WEBHOOK_URL);
  ...
  ])('合法 webhook 變體 %s 通過', (url) => {
    expect(() => validateEnv({ ...validEnv, DISCORD_WEBHOOK_URL: url })).not.toThrow();
  });
  ...
  it.each([
    ...
    expect(() => validateEnv({ ...validEnv, DISCORD_WEBHOOK_URL: url })).toThrow();
  });

  it('擲錯訊息不含機密值', () => {
    try {
      validateEnv({ ...validEnv, DISCORD_WEBHOOK_URL: 'https://evil.com/x' });
      fail('應擲錯');
    } catch (e) {
