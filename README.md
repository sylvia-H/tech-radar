...
名額 10，第 6 關把關）；其餘才丟。起因是 2026-09-15 Jev 發布：HN 1979 分的官方公告在四天視窗內每天被抓到、
...
| 未歸類名額 | 第 4 關保留的「未歸類」候選須有分數且 ≥300，依分數只取前 **10** 則（2026-09-25） |
| 收斂 | 取前 **70** 則送 LLM（2026-09-25 由 60 調高，容納未歸類名額） |
...
1. **通道而非救援**：關鍵字無命中但分數 ≥300 的候選以「未歸類」身分入池（每日 10 席），由 LLM 判定領域與
...
配套：候選池 60 → 70、晨報上限 10 → 15（非 AI 預設 5），上限是天花板不是目標；降級路徑排除未歸類候選。
