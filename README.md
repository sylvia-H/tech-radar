...
比對 `seenNews` 已推清單（保留 7 天，讀寫兩端都修剪），比對鍵同樣是正規化後的 URL，所以換了追蹤
...
| 步驟 | 規則 |
|------|------|
| 分數門檻 | 只對**有**分數者生效：Tier 1 ≥100、Tier 3 ≥150；Tier 2 無門檻 |
...
| 排序 | `加權分數 ↓ → 跨來源輪流分配序 ↑ → normalizedUrl ↑`；`publishedAt` 刻意不當決勝鍵 |
| 收斂 | 取前 **50** 則送 LLM |
